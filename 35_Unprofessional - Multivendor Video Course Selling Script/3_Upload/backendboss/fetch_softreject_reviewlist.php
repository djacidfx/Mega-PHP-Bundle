...
$Statement = $pdo->prepare("SELECT * FROM ot_users_temp_video WHERE upload_success = '1' and item_soft_reject = '1' order by item_time asc");
$Statement->execute(); 
...
if($total > 0) {
	...
	foreach($result as $row) {
        ...
        $action = '<a href="'.ADMIN_URL.'tempsoftreview/'.$tempId.'" class="btn btn-sm btn-primary "><i class="fa fa-eye d-inline"></i>Review</a>';
        $softRejects = _e($row['soft_reject_count'])." Times" ;
		$output['data'][] = array( 	
		$sum,	
        $tempId,
        $userId,
