...
$Statement = $pdo->prepare("SELECT count(item_sale) as c, item_Id, item_name, item_preview,item_thumbnail, regular_price, created_date, updated_date, item_sale, item_loved_by, item_status FROM item_db WHERE item_status = '1' and item_sale > 0 group by item_Id order by item_sale desc");
$Statement->execute(); 
...
if($total > 0) {
	...
	foreach($result as $row) {
		...
		$sale = '<b class="text-success">'._e($row['item_sale']).'</b>' ;
