...
<div class="app-title">
		...
		if(isset($total_item) > 0){ ?>
		  ...
		  <p class="text-success">Requirements (Item Name, Price, Item Description, Category, Tags.)</p>
		<?php } else { ?>
		   <h1><i class="fa fa-upload"></i> Upload Item</h1>
		   <p class="text-success">Requirements (Item Name, Price, Item Description, Category, Tags, Preview & Thumbnail Image, Main File & Video Preview).</p>
		<?php } ?>
...
<form method="post" enctype="multipart/form-data" class="step1formedit">
	...
	<div class="row mt-2">
		...
		<div class="col-lg-12">
		<textarea name="item_tag" id="item_tag" class="form-control" placeholder="Example : Video, After Effects, Motion Graphic Video, etc..." autofocus required><?php echo $itemTags ; ?></textarea>
		</div>
	</div>
	<div class="row mt-2">
		<div class="col-lg-12">
...
<form method="post" enctype="multipart/form-data" class="step1form">
	...
	<div class="row mt-2">
		...
		<div class="col-lg-12">
		<textarea name="item_tag" id="item_tag" class="form-control" placeholder="Example : Video, After Effects, Motion Graphic Video, etc..." autofocus required></textarea>
		</div>
	</div>
	<div class="row mt-2">
		<div class="col-lg-12">
...
<form  method="post" id="uploadFilesNew" class="uploadFilesNew">
	...
	<!--Preview & Thumbnail Image Start-->
			...
			<small>Video Preview Format - .mp4, .mov, .wmv, .flv, .avi, .webm, .mkv, .mpeg, .ogg, .mpg, .mpv, .m4p, .m4p, .m4v, .qt, .swf & .avchd</small>
			</div>
	...
	<!--Main File & Documentation File Start-->
			...
			<div class="col-lg-6 col-md-6">
				<div class="form-group dcmntn">
					<label>Video Preview<?php if(isset($total_item) == 0){ ?>* <?php } ?><small>(Only 256 MB Allowed)</small></label>
					<input type="file" name="uploadDocumentation" id="uploadDocumentation" class="form-control" accept="video/*" <?php if(isset($total_item) == 0){ ?> required <?php } ?> />
				</div>
