...
jQuery(function ($) {
	...
	$(document).on('click', '#add_subscription', function(){
		...
		$('.modal-title').html("<i class='fa fa-plus'></i> Add Plan");
		$('#action_subscribe').val('Add Plan');
	...
	$(document).on('click', '.editSubscription', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('.modal-title').html("<i class='fa fa-pencil-alt'></i> Edit Plan");
				$('#action_subscribe').val('Edit Plan');
	...
	$(document).on('click', '.changeSubscriptionStatus', function(){
			...
			if(confirm("Are you sure you want to change Plan Status?"))
			{
				$.ajax({
					url: base_url+"action_subscription.php",
					method:"POST",
