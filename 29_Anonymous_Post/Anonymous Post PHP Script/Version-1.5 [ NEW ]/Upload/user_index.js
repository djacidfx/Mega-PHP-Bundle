...
jQuery(document).ready(function($) {
    ...
    $(document).on('click','.show_more_all_featured',function(){
        var ID = $(this).attr('id');
        $('.show_more_featured_item').hide();
        $('#loader-icon').show();
        $.ajax({
            type:'POST',
            url: newbase_url+'getNewFeatured.php?id='+ID,
            data:'id='+ID,
            success:function(html){
				$('#loader-icon').hide();
    ...
    $(document).on('click','.show_more_all_trending',function(){
        var ID = $(this).attr('id');
        $('.show_more_trending_item').hide();
        $('#loader-icon').show();
        $.ajax({
            type:'POST',
            url: newbase_url+'getNewTrending.php?id='+ID,
            data:'id='+ID,
            success:function(html){
				$('#loader-icon').hide();
    ...
    $(document).on('click','.show_more_all_popular',function(){
        var ID = $(this).attr('id');
        $('.show_more_popular_item').hide();
        $('#loader-icon').show();
        $.ajax({
            type:'POST',
            url: newbase_url+'getNewPopular.php?id='+ID,
            data:'id='+ID,
            success:function(html){
				$('#loader-icon').hide();
                $('#show_more_popular_item'+ID).remove();
				$('.ap'+ID).remove();
                $('.jQueryNewPost').append(html);
            }
        });
