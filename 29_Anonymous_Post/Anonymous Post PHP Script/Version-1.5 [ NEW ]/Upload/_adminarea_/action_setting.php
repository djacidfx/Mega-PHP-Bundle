...
if(isset($_POST['msg_submit_pr'])){
	if($_POST['msg_submit_pr'] == 'Submit') {
	$alreadyLiked = filter_var($_POST['alreadyLiked'], FILTER_SANITIZE_STRING) ;
	$alreadyLoved = filter_var($_POST['alreadyLoved'], FILTER_SANITIZE_STRING) ;
	$pendingApproval = filter_var($_POST['pendingApproval'], FILTER_SANITIZE_STRING) ;
	$upd = $pdo->prepare("update ot_admin set already_liked = '".$alreadyLiked."', already_loved = '".$alreadyLoved."' , approve_message = '".$pendingApproval."' where id = '1'") ;
	$upd->execute();
	
	$form_message = "Message Template Updated Successfully.";
		$output = array( 
						'form_message' => $form_message
					) ;
		echo json_encode($output);
...
if(isset($_POST['post_submit_pr'])){
	...
	$anony = filter_var($_POST['anony'], FILTER_SANITIZE_NUMBER_INT) ;
	$titleLimit = filter_var($_POST['titleLimit'], FILTER_SANITIZE_NUMBER_INT) ;
	$descriptionLimit = filter_var($_POST['descriptionLimit'], FILTER_SANITIZE_NUMBER_INT) ;
	$upd = $pdo->prepare("update ot_admin set auto_approve = '".$anony."' , title_limit = '".$titleLimit."' , description_limit = '".$descriptionLimit."'   where id = '1'") ;
	$upd->execute();
	
	$form_message = "Post Setting Updated Successfully.";
		$output = array( 
						'form_message' => $form_message
					) ;
		echo json_encode($output);
