...
$Statement = $pdo->prepare("SELECT * FROM anony_post WHERE post_status='1' and post_popular = '1' order by id desc");
$Statement->execute(); 
...
if($total > 0) {
	...
	foreach($result as $row) {
        ...
        $actionBtn = '
                    ...
                    <br>
                    <button type="button" name="changeUnPopularStatus" id="'.$postId.'" class="mt-n2 btn btn-danger btn-block changeUnPopularStatus">MakeUnPopular</button>        
                    ' ;
		if($featured == '1') {
