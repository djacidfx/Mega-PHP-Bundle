	...
	if($_POST['btn_action_sb'] == 'Post')
	{
		...
		if($responseKeys["success"]) {
			...
			if($postDuplicate > 0) {
				...
				$ins = $pdo->prepare("insert into anony_post (post_title,post_description,post_date,post_status, p_user_ip)  values ('".$postTitle."', '".$postDescription."' ,'".$date."' , '".$approveValue."' , '".$ip."')");
				$ins->execute();
