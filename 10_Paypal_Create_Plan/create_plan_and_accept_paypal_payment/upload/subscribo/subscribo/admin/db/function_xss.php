...
{	
	$curday = date('Y-m-d');
	$query = "SELECT * FROM payments WHERE payment_status='Completed' and pay_date = '".$curday."'";
	$statement = $pdo->prepare($query);
	$statement->execute();
	$total = $statement->rowCount();
	return _e($total) ;
...
{	
	$firstday = date('Y-m-01');
	$lastday = date('Y-m-t') ;
	$query = "SELECT * FROM payments WHERE payment_status='Completed' and pay_date between '".$firstday."' and '".$lastday."'";
	$statement = $pdo->prepare($query);
	$statement->execute();
	$total = $statement->rowCount();
	return _e($total) ;
