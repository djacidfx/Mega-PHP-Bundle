	...
	mainDocument.on('click', '.add_wallet_plan', function(){
	 ...
	 $(document).on('change','#uploadDocumentation', function(event){
		...
		if($('#uploadDocumentation').val()) {
				...
				if(FileSize < 50.1) {
					...
					$.ajax({
						...
						success:function(data){
							...
							$('.remove-messagesdocumentation').fadeIn().html('<div class="alert alert-success">Audio Preview File Uploaded Successfully.</div>');
							$('.dcmntn').hide();
					...
					alert("Preview Audio File must not be greater than 50 MB.") ;
			...
			alert("Please Select a .mp3, .ogg, .wav or .m4a file.") ;
			$('#uploadDocumentation').val('');
			$('.documentationprogress').hide();
			return false;
		}
