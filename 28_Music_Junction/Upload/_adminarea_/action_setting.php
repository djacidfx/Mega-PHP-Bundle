...
if(isset($_POST['color_submit_pr'])){
	if($_POST['color_submit_pr'] == 'Submit') {
	$headerColor = filter_var($_POST['headerColor'], FILTER_SANITIZE_STRING) ;
	$footerColor = filter_var($_POST['footerColor'], FILTER_SANITIZE_STRING) ;
	$footerText = filter_var($_POST['footerText'], FILTER_SANITIZE_STRING) ;
	
	$upd = $pdo->prepare("update ot_admin set header_color = '".$headerColor."', footer_color = '".$footerColor."', footer_text_color = '".$footerText."' where id = '1'") ;
	$upd->execute();
	
	$form_message = "Color Settings Updated Successfully.";
		$output = array( 
						'form_message' => $form_message
					) ;
		echo json_encode($output);
