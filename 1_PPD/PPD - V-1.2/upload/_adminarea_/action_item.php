...
if(isset($_POST['btn-action']))
{
	...
	{
		if(!empty($_POST['item_name']) && !empty($_POST['regular_amount']) &&  !empty($_POST['cat']) && !empty($_POST['item_message']) && !empty($_POST['item_tag']) ){
			...
			if(($y_err == 0)) {
				$ins = $pdo->prepare("insert into item_db (item_name, regular_price, main_category, item_description, item_demo_link, item_youtube_link, item_youtube_id,item_tags,created_date, updated_date) values ('".$itemName."', '".$regularAmount."', '".$categoryID."', '".$itemDetails."', '".$demoLink."', '".$you_vid."', '".$youtubeId."', '".$itemTags."', '".$itemDate."', '".$itemDate."')") ;
				$ins->execute();
	...
	{
		if( !empty($_POST['item_id']) && !empty($_POST['item_name']) && !empty($_POST['regular_amount']) &&  !empty($_POST['cat']) && !empty($_POST['item_message']) && !empty($_POST['item_tag']) ){
			...
			if(($y_err == 0)) {
				$ins = $pdo->prepare("update item_db set item_name = '".$itemName."' , regular_price = '".$regularAmount."' , main_category = '".$categoryID."', item_description = '".$itemDetails."' , item_demo_link = '".$demoLink."' , item_youtube_link = '".$you_vid."' , item_youtube_id = '".$youtubeId."' ,item_tags = '".$itemTags."' , updated_date = '".$itemDate."' where item_id = '".$item_id."'") ;
				$ins->execute();
