...
function item_preview($pdo,$itemId){
	...
	foreach($result as $row)
	{
		$output .= '<div class="col-lg-3"><small class="text-muted">Preview</small><br><a href="'.BASE_URL.'itempreview/'.$itemId.'" class="btn btn-primary buttonCircle btn-sm"><i class="fa fa-bug"></i> </a></div>';
	}
	return ($output);
}
function get_item_youtube_video($pdo,$itemId){
	...
	foreach($result as $row)
	{
		...
		$output .= '<div class="col-lg-3"><small class="text-muted">Video</small><br><button type="button" name="viewVideo" id="'.$yid.'" class="btn btn-danger buttonCircle btn-sm viewVideo "><i class="fa fa-youtube-play "></i></button></div>';
	}
	return ($output);
}
...
function top_loved_item_on_index_page($pdo) {
	$limit = "3";
	$query = "SELECT * FROM item_db WHERE item_status = '1' order by item_loved_by DESC LIMIT ".$limit."";
	$statement = $pdo->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	$output = '';
...
function top_downloaded_item_on_index_page($pdo) {
	$limit = "3";
	$query = "SELECT * FROM item_db WHERE item_status = '1' order by item_sale DESC LIMIT ".$limit."";
	$statement = $pdo->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	$output = '';
