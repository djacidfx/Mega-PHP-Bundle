...
jQuery(function ($) {
	...
	$(document).on('submit','.forgot_form', function(event){
		...
		$.ajax({
			url: newbase_url+"verify_email.php",
			method:"POST",
			data:form_data,
	...
	$(document).on('submit','#forgot_otpform', function(event){
		...
		var form_data = $(this).serialize();
		$.ajax({
			url: newbase_url+"verify_password_otp.php",
			method:"POST",
			data:form_data,
			success:function(data)
			{
	...
	$(document).on('submit','#forgotpassword_otpform', function(event){
		...
		var form_data = $(this).serialize();
		$.ajax({
			url: newbase_url+"change_forgot_password.php",
			method:"POST",
			data:form_data,
			success:function(data)
			{
	...
	$(document).on('submit','.signup_form', function(event){
		...
		var form_data = $(this).serialize();
		$.ajax({
			url: newbase_url+"verify_registration.php",
			method:"POST",
			data:form_data,
			success:function(data)
			{
				$('.signup_form')[0].reset();
				if(data == 1) {
					$('.remove-messages').fadeIn().html('<div  class="alert alert-danger errorMessage"><small>Password must contain minimum 8 characters, 1 Uppercase character, 1 Lowercase character & 1 number.</small><button type="button" class="close float-right" aria-label="Close" > <span aria-hidden="true" id="hide">&times;</span></button></div>');
				} 
				if(data == 2) {
	...
	$(document).on('submit','#login_form', function(event){
		...
		var form_data = $(this).serialize();
		$.ajax({
			url: newbase_url+"verify_login.php",
			method:"POST",
			data:form_data,
			success:function(data)
			{
				...
				if(data == 0) {
					...
					$("#action_log").html("<i class='fa fa-sign-in'></i> Login");
